...
const queries = [
  ...
  `CREATE POLICY "tasks_delete_policy" ON tasks FOR DELETE TO authenticated USING (
    EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role IN ('admin', 'manager'))
  );`,

  // RLS Policies for task_checklist_progress
  `CREATE POLICY "task_checklist_progress_select_policy" ON task_checklist_progress FOR SELECT TO authenticated USING (true);`,
  `CREATE POLICY "task_checklist_progress_insert_policy" ON task_checklist_progress FOR INSERT TO authenticated WITH CHECK (true);`,
  `CREATE POLICY "task_checklist_progress_update_policy" ON task_checklist_progress FOR UPDATE TO authenticated USING (true);`,
  ...
  BEGIN
    ...
    INSERT INTO tasks (area_id, bed_id, booking_id, title, task_type, status, priority, due_date)
    VALUES (p_area_id, p_bed_id, p_booking_id, v_title, 'checkout', 'pending', 'high', CURRENT_DATE)
    RETURNING id INTO v_task_id;

    INSERT INTO task_checklist_progress (task_id, checklist_item_id)
    SELECT v_task_id, id FROM area_checklist_items WHERE area_id = p_area_id;
  ...
  BEGIN
    ...
    WHERE tcp.task_id = p_task_id AND aci.is_required = true AND tcp.is_completed = false;
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    try {
      ...
      if (result.message && result.message.includes('ERROR')) {
        console.error(`Query ${i + 1} failed:`, result.message);
      } else {
